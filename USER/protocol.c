...
//	uint8_t tempstate2;//,tempstate3,tempstate4;tempstate,
	...
	{
		n_sstate = sys->sState;
		mcu_dp_value_update(DPID_TYPE, n_sstate); //VALUE型数据上报;
		delay_ms_tm5(20);
	}
	...
	if((sys->t_navi_work - n_worktime) > 60)
	{
...
static ErrorStatus dp_download_fan_handle(const unsigned char value[], unsigned short length)
{
  ...
  fan_value = mcu_get_dp_download_value_littleindian(value,length);
	...
	{
		case 0:
			if(DUST_MODE_NORMAL != cfg->dust_pwm_lev)
			{
				cfg->dust_pwm_lev = DUST_MODE_NORMAL;
				save_cfg();
				log_printf("auto mode\r\n");
				DUST_MOTOR_RANK2();
		...
		break;
		case 1:
			if(DUST_MODE_TURBO != cfg->dust_pwm_lev)
			{
				cfg->dust_pwm_lev = DUST_MODE_TURBO;
				save_cfg();
		...
		break;
		case 2:
			if(DUST_MODE_SILENT != cfg->dust_pwm_lev)
			{
				cfg->dust_pwm_lev = DUST_MODE_SILENT;
				save_cfg();
...
{
  ErrorStatus ret;
  unsigned long sen_lv = 0xFF;
  sen_lv = mcu_get_dp_download_value_littleindian(value,length);
	if(10 >= sen_lv)
	{
		log_printf("\r\nsensitivity level:%d\r\n", sen_lv);
	}
...
static ErrorStatus dp_download_charge_mode_handle(const unsigned char value[], unsigned short length)
{
  ...
  ret = mcu_dp_value_update(DPID_CHARGE,mode);
  return ret;
}
/*****************************************************************************
函数名称 : dp_download_fan_handle
...
static ErrorStatus dp_download_dock_handle(const unsigned char value[], unsigned short length)
{
	...
	if(SUCCESS == ret)
	{
		ret = mcu_dp_value_update(DPID_RETURN_TO_BASE, dock_able);
	}
	return ret;
}
/*****************************************************************************
...
static ErrorStatus dp_download_floor_handle(const unsigned char value[], unsigned short length)
{
	...
	ErrorStatus ret = SUCCESS;
	//0:关/1:开
	unsigned long floor_sen_able = 0xFF;
